...
SELECT way_id,  
    ...
    FROM lines t1 JOIN ", city0," t2
        ON st_intersects(t1.geom,t2.geom)
	WHERE tags ->> 'highway' IN ('services','service','turning_loop','living_street') 
		) t1
	) t2
	) t3
...
SELECT way_id,  
    ...
    FROM lines t1 JOIN ", city0," t2
        ON st_intersects(t1.geom,t2.geom)
	WHERE tags ->> 'highway' IN ('residential', 'rest_area', 'busway')
		) t1
	) t2
	) t3
...
SELECT way_id,  
    ...
    FROM lines t1 JOIN ", city0," t2
        ON st_intersects(t1.geom,t2.geom)
	WHERE tags ->> 'highway' IN ('tertiary', ' tertiary_link')
		) t1
	) t2
	) t3
...
SELECT way_id,  
    ...
    FROM lines t1 JOIN ", city0," t2
        ON st_intersects(t1.geom,t2.geom)
	WHERE tags ->> 'highway' IN ('primary', 'primary_link', 'secondary', 'secondary_link')
		) t1
	) t2
	) t3
...
SELECT way_id,  
    ...
    FROM lines t1 JOIN ", city0," t2
        ON st_intersects(t1.geom,t2.geom)
	WHERE tags ->> 'highway' IN ('trunk', 'trunk_link')
		) t1
	) t2
	) t3
...
SELECT way_id,  
    ...
    FROM lines t1 JOIN ", city0," t2
        ON st_intersects(t1.geom,t2.geom)
	WHERE tags ->> 'highway' IN ('motorway','motorway_link', 'motorway_junction')
		) t1
	) t2
	) t3
