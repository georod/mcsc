...
df_unique_res <- df_unique %>% 
  ...
  mutate(resistance = ifelse(feature == "linear_feature_rail"& type %in% c('platform','preserved','station','traverser'), 100, resistance))%>%
...
df_unique_res <- df_unique_res %>% 
  ...
  mutate(priority = ifelse(feature == "linear_feature_rail"& type %in% c('platform','preserved','station','traverser'), 24, priority))%>%
