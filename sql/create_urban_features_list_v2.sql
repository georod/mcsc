...
UNION ALL
(SELECT feature, CASE WHEN type is null then 'NULL' else type END AS type, material, size, 'lf_roads_very_very_low_traffic_bf' AS view FROM lf_roads_very_very_low_traffic_bf
)
UNION ALL
...
UNION ALL
(SELECT feature, CASE WHEN type is null then 'NULL' else type END AS type, material, size, 'lf_rails_tram_bf' AS view FROM lf_rails_tram_bf
)
UNION ALL
-- parking_surface
...
UNION ALL
--landuse_park
(SELECT  feature, CASE WHEN type is null then 'NULL' else type END AS type, material, size::numeric,  'landuse_park' AS view FROM landuse_park
)
UNION ALL
-- residential
